...
INSERT instructor (CedulaDeIdentidad, Nombre, Apellido, FechaDeNacimiento, FechaDeIncorporacion) 
...
ALTER TABLE `henry_homework`.`cohorte` 
CHANGE COLUMN `Codigo` `Codigo` VARCHAR(20) NOT NULL ;

INSERT INTO cohorte (IdCohorte, Codigo, IdCarrera, IdInstructor, FechaDeInicio, FechaDeFinalizacion)
...
ALTER TABLE `henry_homework`.`alumno` 
CHANGE COLUMN `CedulaDeIdentidad` `CedulaDeIdentidad` VARCHAR(20) NOT NULL ;

INSERT INTO alumno (IdAlumno, CedulaDeIdentidad, Nombre, Apellido, FechaDeNacimiento, FechaDeIngreso, IdCohorte)
VALUES (1,'274352477','Candice','Rojas','2000-12-10','2020-01-21',1235),
  ...
  SELECT * FROM cohorte;
   ...
   -- cambio la fecha de inicio de la cohorte
  UPDATE cohorte
  SET FechaDeInicio = "2022-05-16"
  WHERE IdCohorte IN (1243);
  -- cambio la fecha de inicio de los alumnos de esa cohorte
  UPDATE alumno
  SET FechaDeIngreso = "2022-05-16"
  WHERE IdCohorte IN (1243);
  ...
  SELECT * FROM alumno
  WHERE IdCohorte IN (1235);
  
  -- 8.
  
  SELECT * FROM alumno
  WHERE IdCohorte IN (1235) AND FechaDeIngreso >= '2019-01-01' AND FechaDeIngreso < '2020-01-01';
  
  -- 9.
  
  -- no funciona
  SELECT alumno.Nombre, Apellido, FechaDeNacimiento, carrera.Nombre
	FROM alumno
	INNER JOIN cohorte
  ...
  SELECT
	...
	FROM alumno AS a
    JOIN cohorte AS c ON a.IdCohorte = c.IdCohorte
    JOIN carrera AS ca ON c.IdCarrera = ca.IdCarrera;
  
  -- a. ¿Que campos permiten que se puedan acceder al nombre de la carrera? 
  -- > IdCohorte y IdCarrera
  ...
  -- ¿Cual de las dos opciones es la manera correcta de hacerlo?, 
  -- ¿Por que?
  
    SELECT a.*
	FROM alumno AS a
	JOIN cohorte AS c ON a.IdCohorte = c.IdCohorte
    WHERE c.IdCarrera LIKE 1;
    
    SELECT a.*
	FROM alumno AS a
	JOIN cohorte AS c ON a.IdCohorte = c.IdCohorte
    WHERE c.IdCarrera NOT LIKE 2;
  
  -- la manera correcta es la primera, 
  -- ya que cuando agreguen una tercera carrera va a 
  -- devolver los alumnos de la tercera carrera y la primera
